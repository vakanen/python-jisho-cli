...
if not CFG["ignore_script_name_mismatch"]:
    ...
    if SCRIPT_NAME != this_script_name:
        ...
        print_warning(
            "(You can turn off this warning in the config "
...
def main():
    ...
    def nonnegative_int(number):
        ...
        if number < 0:
            raise argparse.ArgumentTypeError(
                "Needs to be constructible to an integer >= 0"
            )
        return number
    ...
    def nonnegative_float(number):
        ...
        if number < 0:
            raise argparse.ArgumentTypeError(
                "Needs to be constructible to a float >= 0"
            )
        return number
    ...
    def lookup(phrase):
        ...
        try:
            ...
            if args.decompound:
                ...
                # actual name is.
                probably_timed_out = any(
                    x in repr(err) for x in ("timeout", "timed out")
                )
                if probably_timed_out:
                    raise RuntimeError(
                        "It seems the API connection timed out. You may wish "
                        "to override the timeout limit with --timeout."
                    ) from err
            raise err
        ...
        if resp.status_code != expected_response:
            print_warning(
                f"Warning: Unexpected HTTP response {resp.status_code} "
    ...
    def decompound(lexeme, recurse_depth=0, converter=None):
        ...
        for stem in stems:
            ...
            if stem["orig"] not in [v for k, v in stem.items() if k != "orig"]:
                res += decompound(
                    stem["orig"], recurse_depth=recurse_depth, converter=converter
                )
            else:
                res += stem["orig"]
        return res
    ...
    if args.decompound:
        ...
        if args.decompound_literal:
            ...
            if len(primary_result["data"]) == 0:
                ...
                if word is None:
                    print(
                        "Could not find decompoundable results for "
    ...
    def print_definition(word, reading, num_forms):
        output = (
            "\n"
            + colored(
                "Definition" if num_forms == 0 else "\tAlt. form",
                CFG["success_text_color"],
            )
            + ": "
        )
        if word is not None:
        ...
        for i, definition in enumerate(json_datum["data"], start):
            ...
            for form_num, japanese in enumerate(definition["japanese"]):
                ...
                for _, sense in enumerate(definition["senses"], start=1):
                    ...
                    print(f"\t({pos}):")
                    for j, english in enumerate(sense["english_definitions"], start=1):
                        print(f"\t\t{j}: {english}")
    ...
    for json_datum in json_data:
        ...
        if args.decompound:
            print(
                "Decompounding lexeme from API primary search result as per -d/--decompound."
            )
        elif 0 < args.max_results < num_results:
            print(
                f"Limiting output to {args.max_results} result(s) as per "
                "-m/--max-results."
            )
